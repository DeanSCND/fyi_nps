  ...
  def index
      ...
      SurveyAnswer.select("DISTINCT(practice_id)").where(:run => run).each do |result|
        ...
        logger.debug("Sending report for : " + clinic.name + " :" + clinic.id.to_s)
      	...
      	reportFile1 = File.read("files/reports/" + run.name + "/clinics/report_" + clinic.practice_id.to_s + ".pdf")
        reportFile2 = File.read("files/reports/" + run.name + "/clinics/report_" + clinic.practice_id.to_s + "_with_emails.pdf")
