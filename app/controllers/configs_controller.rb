class ConfigsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @config.save
        Rails.configuration.path = @config.value
    
        format.html { redirect_to @config, notice: 'Clinic was successfully created.' }
        format.json { render action: 'show', status: :created, location: @config }
      else
